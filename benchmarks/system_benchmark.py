...
class SystemBenchmark:
    ...
    def create_realistic_datasets(self):
        ...
        for dataset_name, row_count in sizes.items():
            ...
            self.datasets[dataset_name] = self._create_realistic_data(row_count, dataset_name)
    ...
    def _create_realistic_data(self, row_count: int, dataset_type: str) -> pd.DataFrame:
        ...
        base_data = {
            ...
            'status': np.random.choice(['active', 'inactive', 'pending', 'review'], row_count),
            'region': np.random.choice(['north', 'south', 'east', 'west', 'central'], row_count),
            'compliance_score': np.random.beta(3, 2, row_count),  # Skewed towards higher scores
        ...
        # Apply null values
        null_columns = ['facility_name', 'debt_to_income', 'leverage_ratio', 'monthly_revenue']
        for col in null_columns:
            null_indices = np.random.choice(row_count, size=int(row_count * null_rate), replace=False)
            df.loc[null_indices, col] = None
        
        # Apply invalid values
        invalid_indices = np.random.choice(row_count, size=int(row_count * invalid_rate), replace=False)
        df.loc[invalid_indices, 'risk_score_raw'] = -999  # Invalid scores
        df.loc[invalid_indices[:len(invalid_indices)//2], 'compliance_score'] = 1.5  # Out of range
            ...
            'config': {
                "output": {
                    "columns": {
                        ...
                        "name": {"source": "facility_name", "type": "string", "default": "Unknown"},
                        "revenue_millions": {
                ...
                "output": {
                    "columns": {
                        ...
                        "cash_flow_leverage": {
                            ...
                            "type": "float",
                            "validation": {"required": True, "min_value": 0, "max_value": 20}
                        },
                        "risk_category": {
    ...
    def measure_system_metrics(self) -> SystemMetrics:
        ...
        return SystemMetrics(
            ...
            disk_io_before=process.io_counters()._asdict() if hasattr(process, 'io_counters') else {},
            disk_io_after={}  # Will be filled later
        )
    
    def benchmark_original_processing(self, dataset: pd.DataFrame, config: Dict, runs: int = 3) -> BenchmarkResult:
        ...
        for run in range(runs):
            ...
            try:
                ...
                memory_after = sys_metrics_after.memory_usage_after = psutil.Process().memory_info().rss / 1024 / 1024
        ...
        avg_result = {
            ...
            'successful_cols': statistics.mean(r['successful_cols'] for r in results_info),
        ...
        return BenchmarkResult(
            ...
            memory_after=statistics.mean([sys_metrics_before.memory_usage_before + m for m in memory_usage]),
            memory_peak=max([sys_metrics_before.memory_usage_before + m for m in memory_usage]),
    ...
    def benchmark_enhanced_processing(self, dataset: pd.DataFrame, config: Dict, runs: int = 3) -> BenchmarkResult:
        ...
        for run in range(runs):
            ...
            try:
                ...
                try:
                    ...
                    result = dt.fallback_processor.process_with_fallback(input_df, dt.transformation_engine)
        ...
        return BenchmarkResult(
            ...
            success=avg_result['success'] > 0.5,  # Consider success if majority succeeded
            processing_time=statistics.mean(times),
            memory_before=statistics.mean([sys_metrics_before.memory_usage_before]),
            memory_after=statistics.mean([sys_metrics_before.memory_usage_before + m for m in memory_usage]),
            memory_peak=max([sys_metrics_before.memory_usage_before + m for m in memory_usage]),
    ...
    def run_comprehensive_benchmark(self):
        ...
        total_tests = len(self.datasets) * len(self.scenarios) * 2  # x2 for original vs enhanced
        current_test = 0
        ...
        for dataset_name, dataset in self.datasets.items():
            ...
            for scenario_name, scenario in self.scenarios.items():
                ...
                try:
                    enhanced_result = self.benchmark_enhanced_processing(dataset, config)
                ...
                if original_result.processing_time > 0:
                    overhead_ms = (enhanced_result.processing_time - original_result.processing_time) * 1000
                    overhead_pct = (overhead_ms / (original_result.processing_time * 1000)) * 100
                    print(f"    💡 Overhead: {overhead_pct:+.1f}% ({overhead_ms:+.1f}ms)")
                    
                    # Show reliability improvement
                    if enhanced_result.success and not original_result.success:
                        print(f"    🛡️  Reliability: Enhanced succeeded where original failed!")
                    elif enhanced_result.successful_columns > original_result.successful_columns:
                        improvement = enhanced_result.successful_columns - original_result.successful_columns
                        print(f"    📈 Processed {improvement} more columns successfully")
    ...
    def _analyze_performance(self, original_results: List[BenchmarkResult], enhanced_results: List[BenchmarkResult]):
        ...
        for orig, enh in zip(original_results, enhanced_results):
            if orig.processing_time > 0:
                overhead_pct = ((enh.processing_time - orig.processing_time) / orig.processing_time) * 100
        ...
        for orig, enh in zip(original_results, enhanced_results):
            ...
            if size not in size_groups:
                size_groups[size] = []
            if orig.processing_time > 0:
                overhead = ((enh.processing_time - orig.processing_time) / orig.processing_time) * 100
    ...
    def _analyze_reliability(self, original_results: List[BenchmarkResult], enhanced_results: List[BenchmarkResult]):
        ...
        enhanced_success_rate = sum(1 for r in enhanced_results if r.success) / len(enhanced_results) * 100
        
        print(f"Original Success Rate: {original_success_rate:.1f}%")
        print(f"Enhanced Success Rate: {enhanced_success_rate:.1f}%")
        print(f"Reliability Improvement: {enhanced_success_rate - original_success_rate:+.1f} percentage points")
        ...
        orig_attempted_cols = sum(r.successful_columns + r.failed_columns for r in original_results)
        enh_total_cols = sum(r.successful_columns for r in enhanced_results)
        enh_attempted_cols = sum(r.successful_columns + r.failed_columns for r in enhanced_results)
        ...
        if orig_attempted_cols > 0 and enh_attempted_cols > 0:
            ...
            print(f"  Original: {orig_col_success:.1f}% of columns processed successfully")
            print(f"  Enhanced: {enh_col_success:.1f}% of columns processed successfully")
    
    def _analyze_memory_usage(self, original_results: List[BenchmarkResult], enhanced_results: List[BenchmarkResult]):
        """Analyze memory usage patterns."""
        print(f"\n💾 MEMORY USAGE ANALYSIS")
        print("-" * 50)
        
        orig_memory_deltas = [r.memory_after - r.memory_before for r in original_results]
        ...
        if orig_memory_deltas and enh_memory_deltas:
            ...
            print(f"  Difference: {statistics.mean(enh_memory_deltas) - statistics.mean(orig_memory_deltas):+.1f} MB")
            
            # Memory efficiency
            memory_overhead = statistics.mean(enh_memory_deltas) - statistics.mean(orig_memory_deltas)
            if statistics.mean(orig_memory_deltas) > 0:
                memory_overhead_pct = (memory_overhead / statistics.mean(orig_memory_deltas)) * 100
    ...
    def _analyze_scenarios(self):
        ...
        for result in self.results:
            ...
            if len(parts) >= 2:
                ...
                if scenario_type not in scenario_performance:
                    scenario_performance[scenario_type] = {'original': [], 'enhanced': []}
        ...
        for scenario, results in scenario_performance.items():
            if results['original'] and results['enhanced']:
                orig_avg_time = statistics.mean(r.processing_time for r in results['original'])
                enh_avg_time = statistics.mean(r.processing_time for r in results['enhanced'])
                ...
                orig_success = sum(1 for r in results['original'] if r.success) / len(results['original']) * 100
                enh_success = sum(1 for r in results['enhanced'] if r.success) / len(results['enhanced']) * 100
                print(f"  Reliability: {orig_success:.0f}% → {enh_success:.0f}% success rate")
    ...
    def _generate_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            "performance": {
                ...
                "overhead_range": [min(overheads), max(overheads)] if overheads else [0, 0]
            },
            "reliability": {
                "original_success_rate": sum(1 for r in original_results if r.success) / len(original_results) * 100 if original_results else 0,
                "enhanced_success_rate": sum(1 for r in enhanced_results if r.success) / len(enhanced_results) * 100 if enhanced_results else 0,
                "fallback_usage_rate": sum(1 for r in enhanced_results if r.fallback_used) / len(enhanced_results) * 100 if enhanced_results else 0
            },
