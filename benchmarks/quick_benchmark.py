...
def create_test_data(rows=10000):
    ...
    return pd.DataFrame({
        'id': range(1, rows + 1),
        'name': [f'Record_{i}' if i % 100 != 0 else None for i in range(1, rows + 1)],
...
def benchmark_fallback_processing(data, config, runs=5):
    ...
    for _ in range(runs):
        ...
        start_time = time.perf_counter()
        try:
            result = dt.fallback_processor.process_with_fallback(data, dt.transformation_engine)
...
def main():
    ...
    for size in sizes:
        ...
        print(f"   {original_result['avg_time']*1000:8.1f}ms   "
    ...
    partial_result = benchmark_fallback_processing(problem_data, partial_complex_config, runs=1)
